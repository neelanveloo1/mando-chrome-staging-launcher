...
async function status() {
  ...
  pending = (async () => {
    const local = await chrome.runtime.sendNativeMessage('work.mando.chrome.status', {
      action: 'status',
    });
    ...
    const chromeVersion =
      chromeBrand?.version || navigator.userAgent.match(/Chrome\/([\d.]+)/)?.[1] || null;
    lastResult = {
      protocol: 2,
      runningSha,
      runningVersion,
...
chrome.runtime.onConnect.addListener((port) => {
  ...
  try {
    trusted =
      ...
      new URL(port.sender.url).origin === origin;
  } catch {}
  if (!trusted) {
    port.disconnect();
    return;
  ...
  port.onMessage.addListener(async (message) => {
    ...
    let response;
    try {
      response = { nonce: message.nonce, report: await status() };
    } catch {
      response = {
        nonce: message.nonce,
        error:
          'The local checker is unavailable. Install Mando Chrome 1.2.0, then quit and reopen Chrome.',
      };
    }
    try {
