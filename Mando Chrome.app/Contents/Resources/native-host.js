...
function run(argv) {
  ...
  function reply(value) {
    ...
    output.writeData($.NSData.alloc.initWithBase64EncodedStringOptions(b64, 0));
    output.writeData(body);
  }
  try {
    ...
    const body = input.readDataOfLength(length);
    if (Number(body.length) !== length) throw Error('Truncated');
    const request = JSON.parse(
      ObjC.unwrap($.NSString.alloc.initWithDataEncoding(body, $.NSUTF8StringEncoding)),
    );
    if (request.action !== 'status' || Object.keys(request).length !== 1) throw Error('Request');
    const task = $.NSTask.alloc.init,
      pipe = $.NSPipe.pipe;
    task.launchPath = '/bin/bash';
    task.arguments = [argv[0] + '/live-status.sh'];
    ...
    reply(
      JSON.parse(ObjC.unwrap($.NSString.alloc.initWithDataEncoding(data, $.NSUTF8StringEncoding))),
    );
  } catch {
    reply({ check: 'checker_error', latestSha: null, checkedAt: new Date().toISOString() });
  }
}
